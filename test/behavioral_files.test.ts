...
function featureSuite(id: number): FeatureSuiteItem {
  return { feature_id: id, title: `Feature ${id}`, feature_tag: `unitbob_feature_${id}`, suite_digest: `d${id}`, suite_file: checks(id), runner_manifest: { runner: 'cucumber' } };
}
...
// Spec 52-4, AC 1.8. A feature's checks on disk may have been rewritten by the
// host and not saved; the union would wipe that work. So the files are
// compared first, by content, and a difference stops everything before the
// disk is touched. A file that is not on disk at all is not a change — the
// union writes it, as it always did.
test('changedFeatureFiles names a feature whose file on disk differs, and not one whose files match or are absent', () => {
  const projectRoot = tmpProject();
  materializeBehavioral(projectRoot, artifact(), 'cucumber', [checks(12), checks(15)]);
  writeFileSync(join(projectRoot, BEHAVIORAL_DIR, 'step_definitions', 'feature_12_steps.rb'), '# 12, rewired\n');
  rmSync(join(projectRoot, BEHAVIORAL_DIR, 'features', 'feature_15.feature'));

  assert.deepEqual(changedFeatureFiles(projectRoot, [featureSuite(12), featureSuite(15), featureSuite(17)]), [
    { feature_id: 12, title: 'Feature 12', paths: ['.unitbob/behavioral/step_definitions/feature_12_steps.rb'] },
  ]);
});

test('the union stops on a changed feature file before clearing anything, in the connector’s words', () => {
  const projectRoot = tmpProject();
  materializeBehavioral(projectRoot, artifact(), 'cucumber', [checks(12)]);
  const stale = join(projectRoot, BEHAVIORAL_DIR, 'features', 'stale.feature');
  writeFileSync(stale, 'Feature: stale\n');
  const rewired = join(projectRoot, BEHAVIORAL_DIR, 'step_definitions', 'feature_12_steps.rb');
  writeFileSync(rewired, '# 12, rewired\n');
  const index: SuiteIndex = {
    suites: [{ suite_kind: 'behavioral', status: 'ready', suite_digest: 'm', suite_file: artifact(), runner_manifest: { runner: 'cucumber' } }],
    feature_suites: [featureSuite(12)],
  };

  assert.throws(
    () => materializeBehavioralUnion(projectRoot, index, 'cucumber'),
    (err: unknown) =>
      err instanceof FeatureFilesChangedError &&
      err.featureId === 12 &&
      err.message === 'The checks for “Feature 12” changed on disk since they were saved. Run `npx unitbob put-tests 12` to save them, then try again.',
  );
  assert.equal(readFileSync(rewired, 'utf8'), '# 12, rewired\n', 'the rewired file is untouched');
  assert.ok(existsSync(stale), 'nothing was cleared');
});

test('a feature item from a server without titles is named by its id', () => {
  const projectRoot = tmpProject();
  materializeBehavioral(projectRoot, artifact(), 'cucumber', [checks(12)]);
  writeFileSync(join(projectRoot, BEHAVIORAL_DIR, 'features', 'feature_12.feature'), 'Feature: edited\n');
  const untitled = { ...featureSuite(12), title: undefined };

  assert.throws(
    () => materializeBehavioralUnion(projectRoot, { suites: [], feature_suites: [untitled] }, 'cucumber'),
    /The checks for “feature 12” changed on disk/,
  );
});
