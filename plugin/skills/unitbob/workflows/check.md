...
1. `npx -y --loglevel=error unitbob@0.1.11 run` — this fetches the current suite,
   ...
   message and writes nothing — regenerate the suite by following `suite.md` next to this file.
...
invent a cause. If it persists, regenerating the suite (follow `suite.md` next to this
file) is the fix.
