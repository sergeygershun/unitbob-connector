...
Do this:
1. Run `npx -y --loglevel=error unitbob@0.3.1 suite-prepare` with exactly one
...
5. Run `npx -y --loglevel=error unitbob@0.3.1 validate-build`. It checks your
...
6. If no behavioral candidate was built (or it carries `build_error`), skip the
   ...
   Otherwise run `npx -y --loglevel=error unitbob@0.3.1 suite-review-prepare`. It reads the
...
8. Run `npx -y --loglevel=error unitbob@0.3.1 put-suite-build`. It uploads both
