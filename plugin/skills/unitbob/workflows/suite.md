...
1. Run `npx -y --loglevel=error unitbob@0.7.12 suite-prepare` with exactly one
...
5. Run `npx -y --loglevel=error unitbob@0.7.12 accept-worker-plan`. If it exits
...
6. Add what you established about this project to the checkpoints step 5 seeded.
   ...
   `npx -y --loglevel=error unitbob@0.7.12 validate-worker-checkpoints` here,
...
7. Use the same named role on both Claude Code and Codex: `unitbob:suite-worker`
   ...
   **In one go means one message.** All of a branch's workers are launched by a
   ...
   `npx -y --loglevel=error unitbob@0.7.12 codex-install`, then tell the user to
...
8. Run `npx -y --loglevel=error unitbob@0.7.12 validate-worker-checkpoints` after
...
10. Run `npx -y --loglevel=error unitbob@0.7.12 validate-build` after assembly. It
...
11. Run `npx -y --loglevel=error unitbob@0.7.12 run-local` once for the assembled
...
12. Launch one fresh named repair role per failure packet:
    ...
    **The structural branch is finished at that point — publish it now.** Run
    `npx -y --loglevel=error unitbob@0.7.12 put-suite-build structural`. It needs no
...
13. If behavioral is a `build_error`, skip review and keep the structural peer.
    Otherwise run
    `npx -y --loglevel=error unitbob@0.7.12 suite-review-prepare`. It runs and binds
...
15. Run `npx -y --loglevel=error unitbob@0.7.12 put-suite-build behavioral`. It
