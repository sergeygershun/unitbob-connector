...
Do this:
1. Run `npx -y --loglevel=error unitbob@0.7.6 contract-prompt <suite_digest> <test_id> [fix|accept]`.
...
3. Decide and act:
   ...
   - **Accept (the behaviour changed on purpose):** change **only this contract's**
     ...
     run `npx -y --loglevel=error unitbob@0.7.6 suite-review-prepare`, send the new
     ...
     `npx -y --loglevel=error unitbob@0.7.6 put-suite-build`, which publishes the
