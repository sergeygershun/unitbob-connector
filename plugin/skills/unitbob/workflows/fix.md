...
Do this:
1. Run `npx -y --loglevel=error unitbob@0.2.0 contract-prompt <suite_digest> <test_id> [fix|accept]`.
...
3. Decide and act:
   ...
   - **Accept (the behaviour changed on purpose):** change **only this contract's**
     ...
     republish with `npx -y --loglevel=error unitbob@0.2.0 put-suite-build`.
