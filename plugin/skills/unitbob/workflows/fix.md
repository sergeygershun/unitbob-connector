...
Do this:
1. Run `npx -y --loglevel=error unitbob@0.7.12 contract-prompt <suite_digest> <test_id> [fix|accept]`.
...
3. Decide and act:
   ...
   - **Accept (the behaviour changed on purpose):** change **only this contract's**
     ...
     run `npx -y --loglevel=error unitbob@0.7.12 suite-review-prepare`, send the new
     ...
     `npx -y --loglevel=error unitbob@0.7.12 put-suite-build`, which publishes the
...
A `<test_id>` of the form `feature_<id>` — the browser's "Fix the Unitbob guard
feature_12" — names the checks of a feature being built, not a lamp on the
map. They have no digest to copy: pass `feature:<id>` in its place
(`contract-prompt feature:12 feature_12 fix`), and the connector resolves the
current version of the feature's checks itself. There is no accept for them:
the feature's scenarios change through the talk (`workflows/knowledge.md`),
never through an edit to the checks, so the only intent is `fix` — the
feature's own code, until every scenario passes.
