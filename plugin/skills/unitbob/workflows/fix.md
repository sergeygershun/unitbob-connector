Act on the red guard the user named — `<guard_id>` below — a business capability whose unit test failed.
...
Do this:
1. Run `npx -y --loglevel=error unitbob@0.1.10 fix-prepare <guard_id>`. This fetches the per-capability
...
3. Decide and act:
   - **Fix (default):** the behaviour should still hold. Edit **only application
     ...
     game the examples. Then ask the user, in plain words, to run the checks again.
   - **Accept (the behaviour changed on purpose):** re-author **only this
     ...
     `npx -y --loglevel=error unitbob@0.1.10 put-suite-build`. This makes a new suite version against
     the same map.
