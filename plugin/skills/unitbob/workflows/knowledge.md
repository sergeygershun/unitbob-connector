...
1. If you do not already know the feature's id from this session, run
   `npx -y --loglevel=error unitbob@0.7.10 knowledge-prepare` with no argument.
...
2. Run `npx -y --loglevel=error unitbob@0.7.10 knowledge-prepare <id>`. It
...
5. Only on an explicit "yes": write `knowledge.md` to the `output_path` the
   ...
   `npx -y --loglevel=error unitbob@0.7.10 put-knowledge <id>`. It records the
...
6. Tell the user in **one short message**: the server's sentence (how many
   ...
   printed. No retelling of the conversation, no file contents.

Do **not** upload without the user's "yes", do **not** start implementing, and
do **not** run any other Unitbob workflow on your own — the tests come later,
from this file, when the user asks.
