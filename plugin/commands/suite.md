...
Do this:
1. Run `npx -y --loglevel=error unitbob@0.1.8 suite-prepare`. This confirms at
   ...
   tell the user and stop. If it reports there is no current map, run
   `/unitbob map` first and stop.
...
4. Run `npx -y --loglevel=error unitbob@0.1.8 put-suite-build` to upload the
