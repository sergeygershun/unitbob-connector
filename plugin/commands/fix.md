...
Do this:
1. Run `npx -y --loglevel=error unitbob@0.1.7 fix-prepare $ARGUMENTS`. This fetches the per-capability
...
3. Decide and act:
   ...
   - **Accept (the behaviour changed on purpose):** re-author **only this
     ...
     `npx -y --loglevel=error unitbob@0.1.7 put-suite-build`. This makes a new suite version against
     the same map.
